...
export default function TableData({loading, updateList, refresh, search}) {
  ...
  const [url, setUrl] = useState(
    "https://nmcnpm.herokuapp.com/api/v2/station/bikes"
  );
  ...
  const handleCheck = (id) => {
    setChecked((prev) => {
      ...
      if (isChecked) {
        ...
        if (checked.length === stringAfterFilter.length - 1)
          setCheckedAll(true);
        return [...prev, id];
      }
    });
        ...
        <div className="dropdown">
          ...
          <div className="dropdown-list">
            <div
              ...
              onClick={() =>
                setUrl(
                  "https://nmcnpm.herokuapp.com/api/v2/station/bikes?type=free"
                )
              }
            >
              Trạng thái: rảnh
            ...
            <div
              ...
              onClick={() =>
                setUrl(
                  "https://nmcnpm.herokuapp.com/api/v2/station/bikes?type=waiting"
                )
              }
            >
              Trạng thái: bận
            ...
            <div
              ...
              onClick={() =>
                setUrl(
                  "https://nmcnpm.herokuapp.com/api/v2/station/bikes?type=breakdown"
                )
              }
            >
              Trạng thái: sửa
            ...
            <div
              ...
              onClick={() =>
                setUrl(
                  "https://nmcnpm.herokuapp.com/api/v2/station/bikes?type=hiring"
                )
              }
            >
              Trạng thái: đã đặt
            </div>
            <div
              className="dropdown-list-item"
              onClick={() =>
                setUrl("https://nmcnpm.herokuapp.com/api/v2/station/bikes")
              }
            >
              Từ giá thuê cao đến thấp
            </div>
            <div
              className="dropdown-list-item"
              onClick={() =>
            ...
            if (
              ...
              return (
                <tr key={index}>
                  ...
                  {post.status == "free" && (
                    <td>
                      <div className="status11">{post.status}</div>
                    </td>
                  )}
                  {post.status == "waiting" && (
                    <td>
      ...
      <div className="contentbarBottom">
        <div className="contentcomment">
          ...
          <div className="contentBold">
            ...
            {(currentPage - 1) * 4 == stringAfterFilter.length ? 0 : 1}-
            {((currentPage - 1) * 4 + 4 < stringAfterFilter.length &&
              (currentPage - 1) * 4 + 4) ||
              stringAfterFilter.length}
          </div>
          &nbsp;from
          <div className="contentBold">
            &nbsp;{stringAfterFilter.length}&nbsp;
        ...
        <div className="contentnumberTab">
          ...
          <FaCaretRight
            className="goicon"
            onClick={() => handleSetPagesUp(pages, stringAfterFilter.length)}
          ></FaCaretRight>
        </div>
      </div>
    </div>
