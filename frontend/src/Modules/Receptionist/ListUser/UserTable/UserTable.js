...
export default function UserTable({loading, updateList, search}) {
  ...
  async function deleteUser(id) {
    ...
    await axios
      .delete(`https://nmcnpm.herokuapp.com/api/v2/user/delete/` + id, {
  ...
  async function activateUser(post) {
    ...
    await axios
      .put(
        `https://nmcnpm.herokuapp.com/api/v2/user/edit/` + post._id,
        userActivated,
        {headers: {Authorization: `Bearer ${token}`}}
      )
      .then((res) => {});
  ...
  async function getData() {
    const token = localStorage.getItem("token");
    await axios
      .get(`https://nmcnpm.herokuapp.com/api/v1/activated/users`, {
        headers: {Authorization: `Bearer ${token}`},
      })
      .then((res) => {
                ...
                <tr key={index}>
                  ...
                  <td>
                    <div className="infor">
                      <img
                        className="avatar1"
                        src={avatar[Math.floor(Math.random() * 1)]}
                      />
                  ...
                  <td>
                    <div className="status11">Pending</div>
                  </td>
                  <td
                    style={{
                      display: "flex",
